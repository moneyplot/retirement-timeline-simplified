...
Standard personal finance paradigms rely on fragile rules of thumb. To achieve absolute mathematical certainty, we model an individual's financial lifecycle across a strictly bounded timeline from a starting age ($A_{\text{start}}$) to a maximum life expectancy ($A_{\text{end}}$). We define the total planning horizon as:
\begin{equation}
H = A_{\text{end}} - A_{\text{start}}
\end{equation}
...
\begin{itemize}
    \item $P_0$: Initial Net Worth / Starting Balance (\$)
    \item $c$: Annual Cash Savings Flow during working years (\$)
    \item $b$: Annual Retirement Budget Consumption (\$)
    \item $P_{\text{death}}$: Desired Capital Legacy Floor / Inheritance Remaining at Death (\$)
    \item $r$: Real Growth Rate adjusted for inflation (expressed as a decimal)
...
Before introducing the compounding curves of financial markets, we establish an intuitive baseline by assuming real asset growth perfectly tracks inflation ($r = 0\%$). In this environment, wealth accumulates and depletes along perfectly straight lines, making it easy to see how the timelines cross.

\subsection{The First-Principles Equations}
During the working phase, the accumulation of assets grows linearly from our starting balance $P_0$ by injecting our cash savings flow $c$ at the end of each year for $w$ years:
\begin{equation}
\text{Peak Assets} = P_0 + c \cdot w
\end{equation}

Upon entering retirement, the accumulation engine shuts off and the depletion engine takes over. The capital depletion loop draws down our peak nest egg by a flat budget spending block $b$ each year for the remaining $f$ retirement years ($H - w$) until landing exactly on our target legacy floor $P_{\text{death}}$:
\begin{equation}
\text{Peak Assets Required} = P_{\text{death}} + b \cdot (H - w)
\end{equation}
...
\begin{equation}
P_0 + c \cdot w = b \cdot (H - w) + P_{\text{death}}
\end{equation}

Distributing the retirement consumption $b$ across the timeline parameters yields:
\begin{equation}
P_0 + c \cdot w = b \cdot H - b \cdot w + P_{\text{death}}
\end{equation}

Grouping all terms containing our target variable $w$ onto the left-hand side:
\begin{equation}
c \cdot w + b \cdot w = b \cdot H + P_{\text{death}} - P_0
\end{equation}

Factoring out $w$ isolates our terms neatly:
\begin{equation}
w(c + b) = b \cdot H + P_{\text{death}} - P_0
\end{equation}

Dividing through by $(c + b)$ yields the beautiful, closed-form \textbf{Money Plot Linear Baseline Equation}:
\begin{equation}
w = \frac{b \cdot H + P_{\text{death}} - P_0}{c + b}
\end{equation}
...
When we introduce real market investment returns ($r > 0\%$), our straight lines transform into curves. During the working phase ($w$), net worth grows via compound interest on the initial principal while absorbing an ordinary annuity flow of annual savings $c$ injected at the end of each period. The net worth at the exact boundary of retirement ($P_w$) is defined by:
\begin{equation}
{\color{mpgreen} P_w} = P_0(1+r)^w + c \cdot \frac{(1+r)^w - 1}{r}
\end{equation}

\subsection{Step-by-Step Inductive Derivation of $P_w$}
...
\begin{align*}
    P_1 &= P_0(1+r) + c \\
    P_2 &= P_1(1+r) + c = \left[P_0(1+r) + c\right](1+r) + c \\
        &= P_0(1+r)^2 + c(1+r) + c \\
    P_3 &= P_2(1+r) + c = \left[P_0(1+r)^2 + c(1+r) + c\right](1+r) + c \\
        &= P_0(1+r)^3 + c(1+r)^2 + c(1+r) + c
\end{align*}

Extending this inductive sequence to a generalized working timeline of $w$ years yields:
\begin{equation}
    {\color{mpgreen} P_w} = P_0(1+r)^w + c \sum_{k=0}^{w-1} (1+r)^k
\end{equation}
...
\begin{align*}
    R_1 &= ({\color{mpgreen} P_w} - b)(1+r) = {\color{mpgreen} P_w}(1+r) - b(1+r) \\
    R_2 &= (R_1 - b)(1+r) = \left[{\color{mpgreen} P_w}(1+r) - b(1+r) - b\right](1+r) \\
        &= {\color{mpgreen} P_w}(1+r)^2 - b(1+r)^2 - b(1+r) \\
    R_3 &= (R_2 - b)(1+r) = \left[{\color{mpgreen} P_w}(1+r)^2 - b(1+r)^2 - b(1+r) - b\right](1+r) \\
        &= {\color{mpgreen} P_w}(1+r)^3 - b(1+r)^3 - b(1+r)^2 - b(1+r)
\end{align*}

Extending this progression to a total retirement duration of $f$ years yields:
\begin{equation}
    R_f = {\color{mpgreen} P_w}(1+r)^f - b \sum_{k=1}^{f} (1+r)^k
\end{equation}
...
Distributing the factored $(1+r)$ back across the numerator yields the completed, time-synchronized depletion engine expression:
\begin{equation}
    {\color{mpblack} R_f} = {\color{mpgreen} P_w} {\color{mpred}(1+r)^f} - {\color{mpred}b \cdot \frac{(1+r)^{f+1} - (1+r)}{r}}
\end{equation}

---

\section{The Unified Derivation with Legacy Capital Floors}
To complete our global solution, we enforce our final boundary constraint where our asset base at the end of life must exactly match our legacy target ($R_f = P_{\text{death}}$). By substituting our timeline identity $f = H - w$, we can map our accumulation peak directly into our depletion model.
...
\begin{equation}
\left[ {\color{mpgreen} P_0(1+r)^w + c \cdot \frac{(1+r)^w - 1}{r}} \right] {\color{mpred}(1+r)^{H-w}} - {\color{mpred}b \cdot \frac{(1+r)^{H-w+1} - (1+r)}{r}} = {\color{mpblack}P_{\text{death}}}
\end{equation}
...
\begin{equation}
P_0(1+r)^H + \frac{c}{r}(1+r)^H - \frac{c}{r}(1+r)^{H-w} - b \cdot \frac{(1+r)^{H-w+1} - (1+r)}{r} = P_{\text{death}}
\end{equation}

Multiplying the entire equation by our common denominator $r$ removes fractional complexity:
\begin{equation}
rP_0(1+r)^H + c(1+r)^H - c(1+r)^{H-w} - b(1+r)^{H-w+1} + b(1+r) = rP_{\text{death}}
\end{equation}
...
\begin{equation}
(1+r)^H \left( rP_0 + c \right) + b(1+r) - rP_{\text{death}} = \left[ c + b(1+r) \right] (1+r)^{H-w}
\end{equation}

Isolating our exponential timeline block completely:
\begin{equation}
(1+r)^{H-w} = \frac{(rP_0 + c)(1+r)^H + b(1+r) - rP_{\text{death}}}{c + b(1+r)}
\end{equation}

Taking the natural logarithm ($\ln$) of both sides frees our timeline variable from the exponent:
\begin{equation}
(H - w) \ln(1+r) = \ln \left[ \frac{(rP_0 + c)(1+r)^H + b(1+r) - rP_{\text{death}}}{c + b(1+r)} \right]
\end{equation}

Isolating $w$ delivers the final \textbf{Money Plot Master Legacy Equation}:
\begin{equation}
w = H - \frac{\ln \left[ \frac{(rP_0 + c)(1+r)^H + b(1+r) - rP_{\text{death}}}{c + b(1+r)} \right]}{\ln(1+r)}
\end{equation}
