...
\begin{itemize}
    ...
    \item $s$: Annual Savings Flow (\$)
    \item $c$: Annual Consumption Outflow (\$)
    \item $w$: Accumulation Years (Work)
    \item $f$: Decumulation Years (Freedom)
    \item $r$: Inflation-Adjusted Real Growth Rate
    \item $A_0$: Initial Net Worth / Balance (\$)
    \item $L$: Desired Legacy Floor Target (\$)
\end{itemize}
\end{multicols}
...
\begin{equation}
\text{Peak Assets Case 1} = s \cdot w
\end{equation}

Upon entering retirement, accumulation stops decumulation begins draining our peak nest egg by a flat annual budget block $b$ for the remaining $f$ retirement years:
\begin{equation}
\text{Peak Assets Required Case 1} = c \cdot f
\end{equation}
...
w = \frac{c \cdot H}{s + c}
\end{equation}
In the example in Figure 1, a person saving 25\% of their income: $c = \$45,000\text{/year}$, $s = \$15,000\text{/year}$, \\
$T_{start} = \text{25 years}$, $T_{end} = \text{95 years}$.
\begin{align*}
    w &= \frac{\$45,000 \cdot 70 \text{ years}}{\$45,000 + \$15,000}\\
    w &= 52.5 \text{ years}
\end{align*}
...
\begin{equation}
    {\color{mpblack} R_f} = {\color{mpgreen} A_w} {\color{mpred}(1+r)^f} - {\color{mpred}c \cdot \frac{(1+r)^{f+1} - (1+r)}{r}}
\end{equation}
...
\begin{equation}
\left[ {\color{mpgreen} A_0(1+r)^w + s \cdot \frac{(1+r)^w - 1}{r}} \right] {\color{mpred}(1+r)^{H-w}} - {\color{mpred}c \cdot \frac{(1+r)^{H-w+1} - (1+r)}{r}} = {\color{mpblack}L}
\end{equation}
...
\caption{The exponential compounding lifecycle curves where $r>0\%$. The \textcolor{mpgreen}{accumulation phase} curves upward via geometric expansion and transitions to an accelerated annuity-due \textcolor{mpred}{decumulation} phase.}
